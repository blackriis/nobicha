...
test.describe('Branch Management E2E Tests', () => {
  ...
  test('Add New Branch - Complete Workflow', async ({ page }) => {
    ...
    await test.step('Fill branch form with valid data', async () => {
      ...
      await page.fill('[data-testid="branch-name-input"]', testBranchName);
      await page.fill('[data-testid="branch-address-input"]', testBranchAddress);
      await page.fill('[data-testid="branch-latitude-input"]', '13.7563');
      await page.fill('[data-testid="branch-longitude-input"]', '100.5018');
    });

    await test.step('Submit branch form', async () => {
      // Click submit button
      await page.click('[data-testid="submit-branch-btn"]');
      
      // Wait for redirect back to branches list (alert handled by browser)
    ...
    await test.step('Verify new branch appears in list', async () => {
      // Check if new branch is displayed in the list
      await expect(page.locator(`[data-testid="branch-name"]:has-text("${testBranchName}")`)).toBeVisible();
      await expect(page.locator(`[data-testid="branch-address"]:has-text("${testBranchAddress}")`)).toBeVisible();
    });
  });
  ...
  test('Edit Existing Branch - Update Information', async ({ page }) => {
    ...
    await test.step('Update branch information', async () => {
      ...
      await page.fill('[data-testid="branch-name-input"]', `${testBranchName} (แก้ไข)`);
      
      // Clear and update address
      await page.fill('[data-testid="branch-address-input"]', `${testBranchAddress} (แก้ไข)`);
      
      // Update coordinates
      await page.fill('[data-testid="branch-latitude-input"]', '13.7564');
      await page.fill('[data-testid="branch-longitude-input"]', '100.5019');
    });
    ...
    await test.step('Verify updated branch information', async () => {
      ...
      await expect(page.locator(`[data-testid="branch-name"]:has-text("${testBranchName} (แก้ไข)")`)).toBeVisible();
      await expect(page.locator(`[data-testid="branch-address"]:has-text("${testBranchAddress} (แก้ไข)")`)).toBeVisible();
    });
  });
  ...
  test('Branch Management - Error Handling', async ({ page }) => {
    ...
    await test.step('Test invalid coordinate inputs', async () => {
      ...
      await page.fill('[data-testid="branch-name-input"]', 'สาขาทดสอบ');
      await page.fill('[data-testid="branch-address-input"]', 'ที่อยู่ทดสอบ');
      await page.fill('[data-testid="branch-latitude-input"]', 'invalid-latitude');
      await page.fill('[data-testid="branch-longitude-input"]', 'invalid-longitude');
      
      // Try to submit
      await page.click('[data-testid="submit-branch-btn"]');
      
      // Verify validation error for coordinates
      await expect(page.locator('[data-testid="coordinate-validation-error"]')).toBeVisible();
    });
  });
